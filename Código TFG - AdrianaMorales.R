...
ruta_2023 <- "INTRODUZCA_DATOS_2023"
ruta_2024 <- "INTRODUZCA_DATOS_2024"
...
df_train <- df_2023 %>%
  ...
  select(ROA_bin, Ingresos_Explotacion, Empleados,
         Ratio_Liquidez, Ratio_Solvencia, Ratio_Endeudamiento,
         Antiguedad, sector_cnae, NIF) %>%
  na.omit()

# ── 8. CONSTRUCCIÓN DEL TEST (2024) ─────────────────────────────────────────

nifs_solo_2024 <- setdiff(df_2024$NIF, df_2023$NIF)

cat("NIFs solo en 2024:", length(nifs_solo_2024), "\n")
print(nifs_solo_2024)

# ── RECONSTRUIR df_test SIN ESAS 12 EMPRESAS ─────────────────────────────────
...
df_test <- df_2024 %>%
  ...
  filter(!NIF %in% nifs_solo_2024) %>%
  select(ROA_bin, log_Activo, Ingresos_Explotacion, Empleados,
         Ratio_Liquidez, Ratio_Solvencia, Ratio_Endeudamiento,
         Antiguedad, sector_cnae, NIF) %>%
  na.omit()

cat("Test limpio:", nrow(df_test), "empresas\n")

# ── 9. REGRESIÓN LOGÍSTICA BINARIA ────────────────────────────────────────────

modelo_logit <- glm(
  ROA_bin ~ log(Ingresos_Explotacion+1) + Ratio_Endeudamiento + Empleados,
  data   = df_train,
...
arbol_final <- rpart(
  ROA_bin ~ log(Ingresos_Explotacion+1) + Empleados +
    Ratio_Liquidez + Ratio_Solvencia + Ratio_Endeudamiento +
